...
for i=1:FacLen
    for j=1:length(FacultyList)
        if(FacultyDown(i)==FacultyList(j))
            ...
            CheckList1(i)=1;
            for k=1:31
                if((ExMSlots(i,k) ~= FacultyMSlots(j,k)) & (FacultyMSlots(j,k)~=0))
                    ExMSlots(j,k) = FacultyMSlots(j,k) + (ExMSlots(i,k)*10);
                end
                if((ExASlots(i,k) ~= FacultyASlots(j,k)) & (FacultyASlots(j,k)~=0))
                    ExASlots(j,k) = FacultyASlots(j,k) + (ExASlots(i,k)*10);
                end
            end
