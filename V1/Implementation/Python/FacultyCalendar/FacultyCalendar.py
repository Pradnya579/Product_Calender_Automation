...
if __name__ == '__main__':
    SheetName = "Test_vector"
    InputCalendar = "May_Calendar_v0.1 (2).xlsx"
    Initiative = "GENESIS"
    Month = "April"
    Month = Month.upper()
    ...
    ExdFacultyList = []
    for head in range(3,23):
        print(OutputRows[head][1].value)
        if OutputRows[head][1].value:
    ...
    for i in range(len(ExFacultyList)):
        ExSlots.append([])
    for head in range(3,3+len(ExFacultyList)):
        for cell in range(2,126):
            list.append(ExSlots[head-3],OutputRows[head][cell].value)
            ...
            if ExFacultyList[i] == FacultyList[j]:
                ...
                for k in range(124):
                    if ExSlots[i][k] != Slots[j][k]:
                        ...
                        OutputExcel.cell(row=4+i, column=3+k).value = ExSlots[i][k]
                        if ExSlots[i][k]!=0 and ExSlots[i][k]:
                            temp = int(ExSlots[i][k][0])
                            OutputExcel.cell(row=4+i, column=3+k).fill = PatternFill(fill_type='solid', start_color=ColourValues[temp-1], end_color=ColourValues[temp-1])                            
    Range = 0;    
    for i in range(len(FacultyList)):
        if CheckList[i]==0:
            OutputExcel.cell(row=4+len(ExFacultyList)+Range, column=2).value = FacultyList[i]
            for j in range(124):
                OutputExcel.cell(row=4+len(ExFacultyList)+Range, column=3+j).value = Slots[i][j]
                if Slots[i][j]!=0:
                    temp = int(Slots[i][j][0])
                    OutputExcel.cell(row=4+len(ExFacultyList)+Range, column=3+j).fill = PatternFill(fill_type='solid', start_color=ColourValues[temp-1], end_color=ColourValues[temp-1])
            Range = Range + 1
